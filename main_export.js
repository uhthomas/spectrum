...
;(draw = () => {
	...
	let contentWidth = (nr > r ? h * r : w) * devicePixelRatio + .5 | 0
	let contentHeight = (nr < r ? w / r : h) * devicePixelRatio + .5 | 0
	...
	if (arr.some(n => !isFinite(n))) return
...
window.addEventListener('click', () => actx.state !== 'running' && actx.resume(), { once: true })
