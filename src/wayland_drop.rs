...
fn receive_uri_list(
    ...
    if let Err(error) = result {
        ...
        match first_file_from_uri_list(&data) {
            Ok(path) => {
                let _ = event_proxy.send_event(UserEvent::DroppedFile(path));
            }
            Err(error) => {
...
fn first_file_from_uri_list(data: &str) -> Result<std::path::PathBuf, String> {
    let mut unsupported = None;
    for line in data.lines().map(str::trim) {
        if line.is_empty() || line.starts_with('#') {
            continue;
        }
        if let Some(path) = url::Url::parse(line)
            .ok()
...
#[cfg(test)]
mod tests {
    use super::first_file_from_uri_list;
    ...
    fn parses_wayland_uri_lists_and_percent_encoding() {
        ...
        assert_eq!(
            first_file_from_uri_list(data),
            Ok(PathBuf::from("/tmp/a video.mp4"))
        );
    }

    #[test]
    fn rejects_non_file_drops() {
        assert_eq!(
            first_file_from_uri_list("https://example.com/video.mp4"),
            Err("unsupported dropped URI: https://example.com/video.mp4".to_owned())
        );
    }
}
